...
def log_experiment_random_states(random_state_samples, predicted_dists,
                                 obs_min, obs_max, episode_count, image_data=False, state_meanings=None, apply_softmax=False):
    ...
    Params:
        ...
        episode_count: int
            The current episode count/number
        state_meanings: np.ndarray/list
            The corresponding meaning of a state value per state value
            --> not always possible and sensible
            (e.g. for image data)
        apply_softmax: bool
            A flag on whether to apply softmax on the predicted_dists
            or not. Not necessary if last layer
    ...
    metadata = summary_pb2.SummaryMetadata()
    metadata.plugin_data.plugin_name = 'experiment_random_states_state_meanings'
    metadata.data_class = summary_pb2.DATA_CLASS_TENSOR
    tf.summary.write(tag='experiment-episode-{}-state-meanings'.format(episode_count),
                     step=0,
                     tensor=state_meanings,
                     metadata=metadata)
...
def log_action_distribution(actions, episode_count):
    ...
    # _, value_counts = np.unique(action_arr, return_counts=True)
    # metadata = summary_pb2.SummaryMetadata()
    # metadata.plugin_data.plugin_name = 'action_distributions'
    # metadata.data_class = summary_pb2.DATA_CLASS_TENSOR
    # tf.summary.write(tag='action_distributions', step=episode_count, metadata=metadata,
    #                  tensor=value_counts)
